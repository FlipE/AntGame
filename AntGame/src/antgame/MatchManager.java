...
public class MatchManager {
		...
		for(WorldInfo worldInfo : worlds) {			
			for(int i = 0; i < players.size() - 1; i += 1) {
				for(int j = i + 1; j < players.size(); j += 1) {
					...
					// round 2
					AntWorld world2 = new AntWorld(worldInfo.getWorld(), players.get(j).getBrain(), players.get(i).getBrain(), worldInfo.getSeed());
					Match round2 = new Match(world2, players.get(j), players.get(i), "Round 2");
	...
	/**
	 * reset the match manager instance so it can be re-used for the
	 * next match.
	 */
	public void reset() {
		this.players = new ArrayList<PlayerInfo>();
